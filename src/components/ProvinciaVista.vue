    ...
    try {
        ...
        if (response && response.data) {
            provinciaInfo.value = {
                ...
                weather: response.data.weather || { celsius: 'No disponible', conditions: 'No disponible' },
                thumbnail: response.data.thumbnail || 'url-a-imagen-predeterminada.jpg',
            };
        } else {
...
const obtenerGuias = async () => {
    const provincia = route.params.id.toLowerCase().trim();
    try {
        ...
        guiasFiltrados.value = guias.value.filter(guia => guia.provincia?.toLowerCase().trim() === provincia);
