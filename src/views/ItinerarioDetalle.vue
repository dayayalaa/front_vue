...
const rutaDestino = ref('/guiasTarjetas');
...
const obtenerProvincia = async () => {
    ...
    try {
        const response = await axios.get(`https://back-tesis-lovat.vercel.app/arcana/destino/provincia?provincia=${provinciaId}`);
        if (response?.data) {
            provinciaInfo.value = {
            ...
            console.log('Respuesta de provincia:', response);

            const data_id = response.data.data_id;
            obtenerImagenes(data_id);
            datosNoEncontrados.value = false;
        } else {
            provinciaInfo.value = null;
            datosNoEncontrados.value = true;
        }
    } catch (error) {
                    ...
                    <div>
                        ...
                        <p class="text-gray-600 text-sm leading-relaxed">
                            <strong>Hora de salida:</strong> {{
                                itinerario.vueloIda.details.flights[0].departure_airport.time }}
                        </p>
                    </div>
                </div>
                        ...
                        <div>
                            ...
                            <p class="text-gray-600 text-sm leading-relaxed">
                                <strong>Número de vuelo:</strong> {{
                                    itinerario.vueloVuelta.details.flights[0].flight_number }}
                            </p>
                            <p class="text-gray-600 text-sm leading-relaxed">
                                <strong>Hora de salida:</strong> {{
                                    itinerario.vueloVuelta.details.flights[0].departure_airport.time }}
                            </p>
                        </div>
                    </div>
                </div>
